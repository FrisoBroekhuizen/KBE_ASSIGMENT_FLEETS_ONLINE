    ...
    class Depot(Base):
                ...
                if row_width + vehicle.overall_dimensions[1] + self.parking_gap < self.overall_dimensions[1]:
                    ...
                    if type(vehicle).__bases__[0].__name__ == "Tool" or type(vehicle).__name__ == "Tool":
                        ...
                        self.attachable_tools[attachable_tool_index].append(
                            row_height - (max_vehicle_length - vehicle.overall_dimensions[0]))
                    ...
                    row_width = 0
                    path_width = DeterminePathWidth(self, longest_vehicle)
        ...
        def NonAttachableToolStorage(self):
            return Box(width=np.sqrt(self.non_attachable_tools_storage),
                       ...
                       position=translate(self.position, 'x',
                                          self.overall_dimensions[0] - np.sqrt(self.non_attachable_tools_storage), 'y',
                                          self.overall_dimensions[1] - np.sqrt(self.non_attachable_tools_storage)),
                       color="Green")
        ...
        def PlaceMachines(self):
            return Box(quantify=len(self.sorted_machines),
                       ...
                       position=translate(self.position, 'x', self.machine_positions[child.index][0], 'y',
                                          self.machine_positions[child.index][1]),
                       color=("Blue" if type(self.sorted_machines[child.index]).__bases__[0].__name__ == "Tool" or type(
                           self.sorted_machines[child.index]).__name__ == "Tool"
                              else "Yellow"))

        @Part
        def PlaceGhosts(self):
            return Box(quantify=len(self.attachable_tools),
                       width=max(m.overall_dimensions[0] for m in self.sorted_machines) -
                             self.attachable_tools[child.index][0].overall_dimensions[0],
                       ...
                       position=translate(self.position, 'x', self.attachable_tools[child.index][1], 'y',
                                          self.attachable_tools[child.index][2]),
                       color="Black")
